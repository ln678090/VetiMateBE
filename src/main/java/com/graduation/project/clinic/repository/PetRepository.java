...
public interface PetRepository extends JpaRepository<Pet, UUID> {
  ...
  @Query(
      value = "SELECT p FROM Pet p JOIN FETCH p.customer WHERE p.customer.id = :customerId",
      countQuery = "SELECT COUNT(p) FROM Pet p WHERE p.customer.id = :customerId")
  ...
  @EntityGraph(attributePaths = "customer")
  Optional<Pet> findByIdAndCustomerIdAndDeletedAtIsNull(UUID id, UUID customerId);

  @EntityGraph(attributePaths = "customer")
  Page<Pet> findByCustomerIdAndDeletedAtIsNull(UUID customerId, Pageable pageable);

  @EntityGraph(attributePaths = "customer")
  Optional<Pet> findByIdAndCustomerId(UUID id, UUID customerId);
}
