...
public class QueueTicketController {
  ...
  public ResponseEntity<ApiResp<List<QueueTicketDto>>> getTodayQueue(@RequestParam QueueType type) {
    List<QueueTicketDto> tickets = queueTicketService.getTodayQueue(type);
    return ResponseEntity.ok(
        ApiResp.<List<QueueTicketDto>>builder().message("OK").data(tickets).build());
  }
  ...
  public ResponseEntity<ApiResp<QueueTicketDto>> createTicket(
    ...
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(
            ApiResp.<QueueTicketDto>builder()
  ...
  public ResponseEntity<ApiResp<QueueTicketDto>> updateStatus(
      @PathVariable UUID id, @Valid @RequestBody QueueStatusUpdateRequest request) {
