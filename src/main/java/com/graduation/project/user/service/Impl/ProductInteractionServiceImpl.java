...
public class ProductInteractionServiceImpl implements ProductInteractionService {
    ...
    public void toggleFavorite(UUID userId, UUID productId) {
        ...
        if (favoriteRepository.existsById(id)) {
            ...
            UserFavoriteProduct favorite = UserFavoriteProduct.builder()
                    .userId(userId)
                    .productId(productId)
                    .build();
    ...
    public void recordView(UUID userId, UUID productId) {
        ...
        UserViewedProduct viewed = viewedRepository.findById(id).orElseGet(() -> {
            Product product = productRepository.findById(productId)
                    .orElseThrow(() -> new IllegalArgumentException("Product not found"));
            return UserViewedProduct.builder()
                    .userId(userId)
                    .productId(productId)
                    .build();
        });
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<ProductResp> getFavoriteProducts(UUID userId, OffsetDateTime startDate, OffsetDateTime endDate, Pageable pageable) {
        if (startDate != null && endDate != null) {
            return favoriteRepository.findByUserIdAndCreatedAtBetweenOrderByCreatedAtDesc(userId, startDate, endDate, pageable)
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<ProductResp> getRecentlyViewedProducts(UUID userId, OffsetDateTime startDate, OffsetDateTime endDate, Pageable pageable) {
        if (startDate != null && endDate != null) {
            return viewedRepository.findByUserIdAndViewedAtBetweenOrderByViewedAtDesc(userId, startDate, endDate, pageable)
                    .map(viewed -> productMapper.toResp(viewed.getProduct()));
        }
        return viewedRepository.findByUserIdOrderByViewedAtDesc(userId, pageable)
                .map(viewed -> productMapper.toResp(viewed.getProduct()));
    }
}
