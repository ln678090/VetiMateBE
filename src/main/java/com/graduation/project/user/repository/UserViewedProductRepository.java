...
public interface UserViewedProductRepository
    extends JpaRepository<UserViewedProduct, UserViewedProductId> {
  Page<UserViewedProduct> findByUserIdOrderByViewedAtDesc(UUID userId, Pageable pageable);

  Page<UserViewedProduct> findByUserIdAndViewedAtBetweenOrderByViewedAtDesc(
      UUID userId, OffsetDateTime start, OffsetDateTime end, Pageable pageable);

  @Modifying
  @Query(
      value =
          "DELETE FROM user_viewed_products WHERE user_id = :userId AND product_id NOT IN "
