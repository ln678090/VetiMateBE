...
public class RoleAssignmentServiceImpl
  ...
  public void revokeStaffRoles(
    ...
    if (reason == null || reason.isBlank()) {
      throw new IllegalArgumentException(
          "Lý do thu hồi quyền không được để trống");
    }
    ...
    User user = userRepository.findById(targetUserId).orElseThrow(
        () -> new ResourceNotFoundException(
            "Không tìm thấy tài khoản: "
                + targetUserId));
    boolean adminAccount = user.getRoles().stream().anyMatch(
        role -> ROLE_ADMIN.equals(role.getName()));
    if (adminAccount) {
      throw new StaffConflictException(
          "Không thể xử lý tài khoản Admin qua nghiệp vụ nhân sự");
    }

    user.getRoles().removeIf(
        role -> STAFF_ROLES.contains(role.getName()));
  ...
  public void assignStaffRole(
    ...
    if (reason == null || reason.isBlank()) {
      throw new IllegalArgumentException(
          "Lý do thay đổi quyền không được để trống");
    }
    ...
    boolean adminAccount = user.getRoles()
        .stream()
        .anyMatch(
            role -> ROLE_ADMIN.equals(role.getName()));
    ...
    if (adminAccount) {
      throw new StaffConflictException(
          "Không thể thay đổi tài khoản Admin qua nghiệp vụ nhân sự");
    }
    ...
    user.getRoles().removeIf(
        role -> ROLE_USER.equals(role.getName())
            || STAFF_ROLES.contains(role.getName()));
