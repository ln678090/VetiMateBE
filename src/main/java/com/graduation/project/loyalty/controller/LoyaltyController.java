...
public class LoyaltyController {
  ...
  public ResponseEntity<ApiResp<PointsResponse>> getMyPoints(@AuthenticationPrincipal Jwt jwt) {
    return ResponseEntity.ok(
        ApiResp.<PointsResponse>builder()
  ...
  public ResponseEntity<ApiResp<List<TransactionDto>>> getMyTransactions(
    ...
    return ResponseEntity.ok(
        ApiResp.<List<TransactionDto>>builder()
  ...
  public ResponseEntity<ApiResp<List<VoucherDto>>> getAvailableVouchers() {
    return ResponseEntity.ok(
        ApiResp.<List<VoucherDto>>builder()
  ...
  public ResponseEntity<ApiResp<List<UserVoucherDto>>> getMyVouchers(
      @AuthenticationPrincipal Jwt jwt) {
    return ResponseEntity.ok(
        ApiResp.<List<UserVoucherDto>>builder()
  ...
  public ResponseEntity<ApiResp<UserVoucherDto>> redeemVoucher(
      @RequestBody RedeemVoucherReq req, @AuthenticationPrincipal Jwt jwt) {
    return ResponseEntity.ok(
        ApiResp.<UserVoucherDto>builder()
            .data(
                loyaltyService.redeemVoucher(UUID.fromString(jwt.getSubject()), req.getVoucherId()))
            .message("Success")
            .build());
  }
}
